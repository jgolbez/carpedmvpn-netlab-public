...
1. DR/BDR reduce flooding on broadcast networks - routers send LSU updates to DR/BDR (multicast 224.0.0.6) instead of flooding to all neighbors, reducing overhead
2. The adjacency may not form properly; OSPF requires matching network types on both ends of a link for adjacency to establish
3. Use point-to-multipoint for hub-spoke topologies where not all spokes can reach each other directly (partial mesh), such as hub-spoke VPNs or DMVPN
4. Longer timers (30s hello, 120s dead) reduce overhead on slower or less reliable links typical of point-to-multipoint deployments (WAN, satellite, etc.)
5. One Type 2 LSA (generated by the DR for the entire broadcast segment, regardless of how many routers are on it)
6. Priority 0 prevents a router from becoming DR or BDR - useful for low-resource devices or when you want specific routers to handle DR/BDR duties
7. No - point-to-point links have exactly 2 routers, so DR/BDR election is unnecessary and disabled. The neighbor state shows "Full/-" with no DR designation.
